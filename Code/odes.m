...
for v1=2:99
  dydt(v1)=p.k1_pos*p.clpxp*y(v1+200)/(y(v1+200)+p.km1)+p.k1_pos*p.clpxp*y(v1+100)/(y(v1+100)+p.km1)...
      -2*p.k1_neg*y(v1)+(p.D_complex1/y(601)^2)*(y(v1-1)-2*y(v1)+y(v1+1));
end
...
dydt(401:500)=0;
%%%%%%%%%%%add model 2
%%before uncomment, should change total amount to 601
%complex2. 
...
for v5=502:599
    dydt(v5)=p.k3_pos*y(v5-500)*...y(v5+100)
        p.rcda-p.k3_neg*y(v5)+p.D_complex2*(y(v5+1)-2*y(v5)+y(v5-1))/(y(601)^2);
end
