...
for v3=201:300
  ...
  +p.k1_neg*y(v3-200)-p.k1_pos*p.clpxp*y(v3)/(y(v3)+p.km1);
          ...
          dydt(v3)=-p.kd_cpdr*y(v3-200)*y(v3)/(y(v3-200)+p.J2)...
      +p.kcpdr_f_b*y(v3+200)*y(v3-100)-p.kcpdr_b_f*y(v3)...
  +p.k2_pos*y(v3+100)-p.k2_neg*y(v3)...
  +p.k1_neg*y(v3-200)-p.k1_pos*p.clpxp*y(v3)/(y(v3)+p.km1);
    end
end
...
+p.D_cpdrp*(y(v4+1)-2*y(v4)+y(v4-1))/(y(501)^2);
    ...
    dydt(v4)=p.k2_neg*(y(v4-200)+y(v4-100))-p.k2_pos*y(v4)*p.divkp_free/(p.divkp_free+p.J3)...
+p.D_cpdrp*(y(v4+1)-2*y(v4)+y(v4-1))/(y(501)^2);
    end
end
...
% +p.D_cpdrp*(y(399)-y(400))/(y(501)^2);
  dydt(400)=p.k2_neg*(y(400-200)+y(400-100))-p.k2_pos*y(400)*p.divkp_free/(p.divkp_free+p.J3)...
+p.D_cpdrp*(y(399)-y(400))/(y(501)^2);
%%CpdR sticky
dydt(401:500)=0;
